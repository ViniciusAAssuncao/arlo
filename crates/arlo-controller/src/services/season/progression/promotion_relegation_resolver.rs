...
pub async fn resolve_promotion_relegation(
    ...
    let config = get_or_load_league_calendar_config(pool, competition_id).await?.ok_or_else(|| {
        ControllerError::NotFound(
            format!("League calendar config for competition {} not found", competition_id)
        )
    })?;
    ...
    let needs_standings =
        matches!(promotion_rule, LeagueMovementRule::Automatic { .. }) ||
    ...
    if needs_standings {
        ...
        let sorted = standings_pipeline::calculate_and_rank_standings(
    ...
    let promoted_team_ids = match promotion_rule {
        ...
        LeagueMovementRule::PlayoffStage { stage_order_index, count } => {
            let playoff_stage_row = stages
                .iter()
                .find(|s| s.stage_order_index == (stage_order_index as i32))
                .ok_or_else(|| {
                    ControllerError::NotFound(
                        format!("Promotion playoff stage with order index {} not found", stage_order_index)
                    )
                })?;
            ...
            for tie in &ties {
                let winner = tie
                    ...
                    .ok_or_else(|| {
                        ControllerError::Validation(
                            format!("Promotion playoff tie {} has no aggregate winner", tie.id())
                        )
                    })?;
                winners.push(winner);
            }
    ...
    let relegated_team_ids = match relegation_rule {
        ...
        LeagueMovementRule::PlayoffStage { stage_order_index, count } => {
            let playoff_stage_row = stages
                ...
                .ok_or_else(|| {
                    ControllerError::NotFound(
                        format!("Relegation playoff stage with order index {} not found", stage_order_index)
                    )
                })?;
            ...
            for tie in &ties {
                let loser = tie
                    ...
                    .ok_or_else(|| {
                        ControllerError::Validation(
                            format!("Relegation playoff tie {} has no aggregate loser", tie.id())
                        )
                    })?;
                losers.push(loser);
            }
    ...
    Ok(PromotionRelegationOutcome::new(promoted_team_ids, relegated_team_ids))
}
