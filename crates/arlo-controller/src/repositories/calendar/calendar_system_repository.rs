...
use crate::repositories::calendar::models::{CalendarMonthRow, CalendarSystemRow};
use sqlx::SqlitePool;
...
pub async fn get_by_id(pool: &SqlitePool, id: Uuid) -> ControllerResult<Option<CalendarSystem>> {
    let row = sqlx::query_as::<_, CalendarSystemRow>(
        "SELECT id, name, description, leap_units_per_cycle, cycle_length_years, cycle_reference_year, days_per_occurrence, intercalation_placement_kind, intercalation_placement_month_order_index, created_at_unix_seconds FROM calendar_systems WHERE id = ?",
    )
    .bind(id.to_string())
    .fetch_optional(pool)
    .await?;
...
pub async fn list_all(pool: &SqlitePool) -> ControllerResult<Vec<CalendarSystem>> {
    let system_rows = sqlx::query_as::<_, CalendarSystemRow>(
        "SELECT id, name, description, leap_units_per_cycle, cycle_length_years, cycle_reference_year, days_per_occurrence, intercalation_placement_kind, intercalation_placement_month_order_index, created_at_unix_seconds FROM calendar_systems",
    )
    .fetch_all(pool)
    .await?;
    ...
    for sr in system_rows {
        ...
        systems.push(sr.to_domain(months)?);
    }
