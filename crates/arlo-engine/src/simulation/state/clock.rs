...
impl MatchClock {
    pub fn new(format_rules: &MatchFormatRules) -> Self {
        Self {
            ...
            challenges_per_match: format_rules.challenges_per_match(),
    ...
    pub fn advance_seconds(&mut self, delta: f64) -> bool {
        ...
        self.seconds_in_period += delta.max(0.0);
    ...
    pub fn apply_added_time(&mut self, seconds: f64) {
        self.added_time_seconds += seconds.max(0.0);
    }
    ...
    pub fn total_elapsed_seconds(&self) -> f64 {
        let mut total = 0.0;
        let completed_periods = self.period.saturating_sub(1);
        for p in 1..=completed_periods {
            if p <= self.regulation_periods {
                total += self.regulation_period_duration_seconds;
            } else {
                total += self.overtime_period_duration_seconds;
            }
        }
        total + self.seconds_in_period
    }
}
