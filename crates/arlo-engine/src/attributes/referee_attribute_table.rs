...
use arlo_domain::{AttributeKey, Referee};
use serde::{Deserialize, Deserializer, Serialize, Serializer};
use std::collections::HashMap;
use uuid::Uuid;

#[derive(Debug, Clone, Copy, PartialEq)]
pub struct RefereeAttributeTable {
    values: [f64; AttributeKey::COUNT],
}

impl RefereeAttributeTable {
    ...
    pub fn from_referee(referee: &Referee, attribute_keys: &HashMap<Uuid, AttributeKey>) -> Self {
        let mut values = [10.0; AttributeKey::COUNT];
        for attr in referee.attributes() {
    ...
    pub fn from_referee_with_index(referee: &Referee, key_index: &AttributeKeyIndex) -> Self {
        let mut values = [10.0; AttributeKey::COUNT];
...
impl Default for RefereeAttributeTable {
    fn default() -> Self {
        Self::new_default()
    }
}

impl Serialize for RefereeAttributeTable {
...
impl<'de> Deserialize<'de> for RefereeAttributeTable {
