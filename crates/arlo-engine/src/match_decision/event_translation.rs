...
pub fn translate_scoring_decision(decision: &ScoringDecision) -> Option<MatchEvent> {
    match decision {
        ScoringDecision::GoalPoint { team_id, scorer_id, artrine_id, drives_completed, .. } =>
            Some(
                MatchEvent::GoalPoint(
                    GoalPointScored::new(*team_id, *scorer_id, *artrine_id, *drives_completed)
                )
            ),
        ScoringDecision::FieldPoint {
            ...
            Some(
                MatchEvent::FieldPoint(
                    FieldPointScored::new(
                        *team_id,
                        *scorer_id,
                        *territory_advance_mirim,
                        *drives_completed
        ...
        ScoringDecision::FieldGoal { team_id, scorer_id, post, .. } =>
            Some(MatchEvent::FieldGoal(FieldGoalScored::new(*team_id, *scorer_id, *post))),
