...
pub fn resolve_scoring<R: Rng + ?Sized>(
    ...
    let total_drives = touch_ctx.drives_in_series + progression.drives_recorded;
    let total_adv = touch_ctx.possession_advanced_mirins + progression.mirins_advanced;
    ...
    let goalguard = match find_goalguard(&static_ctx.defense_players) {
        Ok(g) => g,
        Err(_) => return ScoringDecision::NoOpportunity,
    };

    let finish_ctx = touch_ctx
        .duel_context
    ...
    let effective_kicker = if matches!(
        ...
        let mut candidates = touch_ctx.target_candidates.clone();
    ...
    let is_home = static_ctx.offense_team_id == state.home_team_id();
    ...
    if touch_ctx.duel_context.attacker_is_home() {
        z_gap += state.tuning().home_advantage_profile.power_z_boost();
    }
    if touch_ctx.duel_context.defender_is_home() {
        z_gap -= state.tuning().home_advantage_profile.power_z_boost();
    }
