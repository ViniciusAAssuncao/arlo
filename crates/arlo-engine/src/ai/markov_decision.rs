...
impl MarkovDecisionEvaluator {
    pub fn evaluate_action_utilities(
        ...
        let current_epv = EpvModel::calculate_epv(
            ...
            let expected_future_epv = match kind {
                ArtrineDecisionKind::SelfCarry => {
                    ...
                    let (new_down, new_rem) = if adv_mirim >= remaining_advance_mirim {
                        ...
                        (down.saturating_add(1).min(4), (remaining_advance_mirim - adv_mirim).max(0.0))
                    };
                    let new_norm_x = (normalized_proximity + adv_mirim / pitch_length_mirim.max(1.0)).min(1.0);
                    let epv_success = EpvModel::calculate_epv(new_norm_x, new_down, new_rem, new_drives);
                    let epv_fail = if down >= 4 && remaining_advance_mirim > 0.0 {
                        -EpvModel::opponent_score_value(normalized_proximity)
                    } else {
                        EpvModel::calculate_epv(normalized_proximity, down.saturating_add(1).min(4), remaining_advance_mirim, drives_in_series)
                    };
                    let p_succ = (pc * 0.75 + 0.15 * skill_mult).clamp(0.10, 0.95);
                    let p_to = ((1.0 - pc) * 0.15).clamp(0.02, 0.30);
                    let p_fail = (1.0 - p_succ - p_to).max(0.0);
                    p_succ * epv_success + p_fail * epv_fail - p_to * EpvModel::opponent_score_value(normalized_proximity)
                }
                ArtrineDecisionKind::ShortPass => {
                    ...
                    let new_norm_x = (normalized_proximity + adv_mirim / pitch_length_mirim.max(1.0)).min(1.0);
                    let epv_success = EpvModel::calculate_epv(new_norm_x, new_down, new_rem, drives_in_series);
                    let epv_fail = if down >= 4 && remaining_advance_mirim > 0.0 {
                        -EpvModel::opponent_score_value(normalized_proximity)
                    } else {
                        EpvModel::calculate_epv(normalized_proximity, down.saturating_add(1).min(4), remaining_advance_mirim, drives_in_series)
                    };
                    let p_succ = (0.50 + 0.04 * pass_protection_net_advantage + 0.20 * target_quality + 0.10 * skill_mult).clamp(0.15, 0.95);
                ...
                ArtrineDecisionKind::LongLaunch => {
                    ...
                    let p_succ = (0.35 + 0.03 * pass_protection_net_advantage + 0.25 * target_quality + 0.10 * skill_mult).clamp(0.10, 0.85);
                ...
                ArtrineDecisionKind::Cross => {
                    ...
                    let p_goal = (0.30 + 0.40 * normalized_proximity + 0.15 * target_quality + 0.10 * skill_mult).clamp(0.10, 0.90);
                    let v_opp = EpvModel::opponent_score_value(normalized_proximity);
                ...
                ArtrineDecisionKind::SelfFinish => {
                    ...
                    let p_goal = (0.25 + 0.50 * normalized_proximity + 0.15 * skill_mult).clamp(0.05, 0.90);
                    let v_opp = EpvModel::opponent_score_value(normalized_proximity);
                    p_goal * value - (1.0 - p_goal) * v_opp
                }
            };
