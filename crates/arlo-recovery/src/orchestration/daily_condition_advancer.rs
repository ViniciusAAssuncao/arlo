...
pub async fn advance_all_players_one_day(
    ...
    for p in players {
            ...
            match progression {
                ...
                InjuryProgressionOutcome::TransitionedToObservation(updated) => {
                    player_injury_history::update_progress(
                        ...
                        updated.id(),
                        0,
                        updated.observation_days_remaining(),
                        "Observation",
                    )
                    .await?;
                }
                ...
                InjuryProgressionOutcome::FullyRecovered => {
                    player_injury_history::mark_resolved(pool, record.id(), now_seconds).await?;
                }
            }
        }
        ...
        player_condition::upsert(pool, &updated_condition).await?;
    }
