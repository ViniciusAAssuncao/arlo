...
pub async fn insert_batch(
    ...
    execute_batch_insert(tx, "match_availability_changes", COLUMNS, rows, |b, row| {
        b.push_bind(&row.id);
...
pub async fn list_by_match_id(
    pool: &SqlitePool,
    match_id: Uuid,
) -> PersistenceResult<Vec<MatchAvailabilityChangeRow>> {
    let rows = sqlx::query_as::<_, MatchAvailabilityChangeRow>(
        r#"SELECT
            id,
            match_id,
            sequence_number,
            ...
            new_status,
            remaining_seconds
        FROM match_availability_changes
        WHERE match_id = ?
