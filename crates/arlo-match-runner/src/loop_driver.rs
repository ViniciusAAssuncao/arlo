...
pub fn run_match_with_inbox<R: Rng + ?Sized>(
    state: &mut MatchState,
...
pub fn run_loop<R: Rng + ?Sized>(
    ...
    run_loop_with_inbox(state, &inbox, raw_sink, aggregators, rng, max_iterations)
}

pub fn run_loop_with_inbox<R: Rng + ?Sized>(
    state: &mut MatchState,
    inbox: &ManagerDecisionInbox,
    raw_sink: &mut InMemorySink,
    aggregators: &mut AggregatorRegistry,
